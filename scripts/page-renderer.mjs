...
export const protectContext = async (context) => {
  ...
  await context.route("**/*", async (route) => {
    try { await assertPublic(route.request().url()); await route.continue(); } catch (error) {
...
export const render = async (args) => {
  ...
  const proxy = await createPinnedProxy(maxDownloadBytes); let browser;
  try {
    ...
    const routeRejection = await protectContext(context); const page = await context.newPage();
    ...
    const rejection = routeRejection() ?? proxy.failure(); if (rejection) throw rejection; await assertPublic(page.url());
